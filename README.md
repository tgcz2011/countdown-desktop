...
- 单实例接管：软件同一时间只运行一个实例；若已有实例在运行，新启动的实例（无论 GUI 还是带参数启动）会自动通知旧实例退出并接管，后启动者覆盖先启动者的效果。
...
- `--exam gaokao` / `--exam zhongkao` 会自动启用壁纸与屏保（预设模式即用于显示倒计时）；若此前关闭过壁纸，不自动启用会导致「选了中考却没壁纸」。显式 `--wallpaper-enabled off` 仍优先，尊重用户主动关闭。
